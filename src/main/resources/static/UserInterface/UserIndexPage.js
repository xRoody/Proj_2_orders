...
async function init_cart(){
    ...
    if(storage.key(0)){
        ...
        sub.addEventListener('click',async () => {
            ...
            if (num!=0) await init_congratulation(num);
            else init_bad_order();
        })
...
async function send_nodes(){
    ...
    for(let i=0; i<storage.length; i++) {
        ...
        let f=true;
        for(let c of obj.characteristics){
            if (c.quantity>c.mainQuantity){
                f=false;
                f1=false;
            }
        }
        if(f){
            for(let ob of obj.characteristics){
                ...
                for(let or of origin){
                    if(ob.title==or.title && ob.quantity!=or.quantity){
                        let change={};
                        change.nodeId=ob.id;
                        change.cardId=i;
                ...
                if(map[ob.title]){
                    map[ob.title]=map[ob.title]+Number(ob.quantity);
                }
                else map[ob.title]=Number(ob.quantity);
            }
        }
    }
    let f3=await minus_from_balance(price)
    if (f1 && await minus_from_char(map)==200 && f3) {
        ...
        if (!f3){
            ...
            await minus_from_char(map)
        }
        return 0;
    }
}

async function minus_from_balance(){
    return true; // is money
}
...
async function init_bad_order(){
    let con_wrapper=document.createElement("div");
    con_wrapper.classList.add("con_wrapper");
    let header=document.createElement("p");
    header.textContent="Incorrect order";
    header.style.fontSize="60px";
    con_wrapper.appendChild(header);
    let button=document.createElement("button");
    button.classList.add("back_button");
...
async function minus_from_char(map){
    let response = await fetch(service_url +"/characteristics", {
        method: 'PATCH',
        headers: {
            'Authorization': 'Bearer ' + access_token,
            'Content-Type': 'application/json'
    ...
    let rez =response.json().then(async ()=>{
        if (rez.E) {
            await refresh();
            return await minus_from_char(map);
        }
    }).catch();
    return st;
